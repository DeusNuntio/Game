...
canvas.width = 10 * 48 + 16;
canvas.height = 7 * 48 + 16;
...
function refresh(): void {
  ...
  objectiveTracker.update(
    (state.mission?.objectives ?? []).map((o) => ({ id: o.id, description: o.description, complete: o.complete })),
  );

  if (missionOver) {
    banner.style.display = 'block';
    banner.textContent =
      state.mission?.status === 'won' ? 'MISSION ERFÜLLT' : 'MISSION GESCHEITERT';
